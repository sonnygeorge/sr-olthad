...
class OlthadTraversal:
    ...
    def __init__(self, task_description: str):
        ...
        self._nodes: Dict[str, TaskNode] = {
            self.root_node.task: self.root_node
        }
        self._next_planned_subtask_idxs: Dict[str, int] = {
            self.root_node.task: 0
        }
    ...
    def update_next_planned_subtask_after_attempt(
        ...
        attempted_subtask_idx = self._next_planned_subtask_idxs[
            self.cur_node.task
        ]
    ...
    def update_planned_subtasks(
        ...
        if self.cur_node.task not in self._next_planned_subtask_idxs:
            assert (
                self.cur_node.subtasks is None
            )  # These conditions should be in sync
        else:
            next_planned_subtask_idx = self._next_planned_subtask_idxs[
                self.cur_node.task
            ]
            ...
            self.cur_node.subtasks = self.cur_node.subtasks[
                :next_planned_subtask_idx
            ]
